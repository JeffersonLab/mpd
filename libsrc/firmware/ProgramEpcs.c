...
#define BITSTREAM_OFFSET	179	/* bytes */
...
int main(int argc, char *argv[])
{
  ...
  int errcnt, cnt, i, epcs_id, slot_lower, slot_upper, verify_only, erase_only, factory;
	uint32_t epcs_addr;
	char x, y, ans[10];
	FILE *fin = NULL;
	int fnMPD;
	...
	while( (i = getopt(argc,argv,"ehvfs:")) != -1 )
	  switch( i )
	    ...
	    case 's': slot_lower = atoi(optarg); break;
	...
	for (k=0;k<fnMPD;k++) // only active mpd set
	{
		...
		if( i >= slot_lower && i <= slot_upper )
		{
			...
			if( verify_only == 0 )
			{
				...
				printf("Erasing 10 sectors starting at address 0x%08x ", epcs_addr);
				fflush(stdout);
