...
/* Readout MPD */
 ...
 for (k=0;k<fnMPD;k++) { // only active mpd set
   ...
   if ( UseSdram ) {
	 ...
	 timeout=1;
	 tiSetBlockLimit(1);
	 printf("WARNING: *** Timeout while waiting for data in mpd %d - check MPD/APV configuration\n", i);
       }
...
/* 	      dma_dabufp++; */
	  ...
	  if ( verbose_level > 1){// && ((iw<500) || ((nwread-iw)<501))) {

	    if (( verbose_level > 2) || (iw<16) || ((nwread-iw)<=(16+nwread%8))) {
	      if ((iw %8)==0) {
		printf("0x%06x:",iw);
	      }
	      printf(" 0x%08x",datao);
...
int DMA_Read(int id, volatile uint32_t *data, int size, int *wrec)
{
  int retVal=0;
