...
#define MAX_SDATA 1024
  ...
  /* Physical memory buffer variables */
...
//  sspInit(7<<19,1<<19,1,iFlag);
  ...
  if (acq_mode & 0x4) { 
     ...
     **************************/
  printf("MPD 0 = %d %d\n", mpdSlot(0), mpdGetEventBuilding(mpdSlot(0)));
...
#define FILE_VERSION 0x1
  ...
  if ((acq_mode & 1) && (mpdGetEventBuilding(mpdSlot(0)) != 0)) { 
    ...
    /* is in OBUF_Read
    if (FastReadout) {
    vmeDmaConfig(2,5,0); // A32, 2sst150
    } else {
    vmeDmaConfig(2,2,0); // A32, BLT32
    }
    */
